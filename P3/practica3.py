...
if __name__ == "__main__":
    ...
    parser.add_argument('--dstIP', dest='dstIP',
                        default=False, help='Dirección IP destino')
    parser.add_argument('--debug', dest='debug', default=False,
                        action='store_true', help='Activar Debug messages')
    parser.add_argument('--addOptions', dest='addOptions', default=False,
                        action='store_true', help='Añadir opciones a los datagranas IP')
    parser.add_argument('--dataFile', dest='dataFile',
                        default=False, help='Fichero con datos a enviar')
    ...
    while True:
        try:
            ...
            if msg == 'q':
                ...
                sendICMPMessage(data, ICMP_ECHO_REQUEST_TYPE, ICMP_ECHO_REQUEST_CODE,
                                8, ICMP_SEQNUM, struct.unpack('!I', socket.inet_aton(args.dstIP))[0])
                ICMP_SEQNUM += 1
            elif msg == '2':
                sendUDPDatagram(data, DST_PORT, struct.unpack(
                    '!I', socket.inet_aton(args.dstIP))[0])
